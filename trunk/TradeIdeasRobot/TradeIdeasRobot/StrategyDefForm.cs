...
namespace TradeIdeasRobot
{
    public partial class StrategyDefForm : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            try { tradeDefinition.DollarsValue = Convert.ToDouble(tbPoints.Text); }
            catch { }
            try { tradeDefinition.PercentsValue = Convert.ToDouble(tbPercent.Text); }
            catch { }

            tradeDefinition.UseProfitTarget = cbTarget.Checked;
            try { tradeDefinition.ProfitTarget = Convert.ToDouble(tbProfitTarget.Text); }
            catch { }
            tradeDefinition.UseStopLoss = cbStoploss.Checked;
            try { tradeDefinition.StopLoss = Convert.ToDouble(tbStoploss.Text); }
            catch { }
            ...
            try { tradeDefinition.BarsTrailStop = Convert.ToDouble(tbBarTrailStop.Text); }
            catch { }
            try { tradeDefinition.PercTrailStop = Convert.ToDouble(tbTrailStop.Text); }
            catch { }

            if (rbExitMinsAfterEntry.Checked)
            {
                tradeDefinition.TimeExitType = ExitTimeType.MinsAfterEntry;
                try { tradeDefinition.MinsAfterEntry = Convert.ToInt32(tbExitMinsAfterEntry.Text); }
                catch { }
            }
            else if (rbExitMinsBeforeClose.Checked)
            {
                tradeDefinition.TimeExitType = ExitTimeType.MinsBeforeClose;
                try { tradeDefinition.MinsBeforeClose = Convert.ToInt32(tbExitMinsBeforeClose.Text); }
                catch { }
            }
            ...
            try { tradeDefinition.StartHrs = Convert.ToInt32(tbStartHrs.Text); }
            catch { }
            try { tradeDefinition.StartMins = Convert.ToInt32(tbStartMins.Text); }
            catch { }
            try { tradeDefinition.EndHrs = Convert.ToInt32(tbEndHrs.Text); }
            catch { }
            try { tradeDefinition.EndMins = Convert.ToInt32(tbEndMins.Text); }
            catch { }

            if (rbShares.Checked)
            {
                tradeDefinition.PositionSizing = PositionSizingType.FixedShares;
                try { tradeDefinition.Sizing = Convert.ToInt32(tbShares.Text); }
                catch { }
            }
            else if (rbPositionSize.Checked)
            {
                tradeDefinition.PositionSizing = PositionSizingType.FixedAmount;
                try { tradeDefinition.Sizing = Convert.ToInt32(tbPositionSize.Text); }
                catch { }
            }
            else if (rbDollarsRisked.Checked)
            {
                tradeDefinition.PositionSizing = PositionSizingType.FixedRisk;
                try { tradeDefinition.Sizing = Convert.ToInt32(tbDollarsRisked.Text); }
                catch { }
            }

            try { tradeDefinition.MaxShares = Convert.ToInt32(tbMaxShares.Text); }
            catch { }
            try { tradeDefinition.SharesIncrement = Convert.ToInt32(tbIncrement.Text); }
            catch { }
            try { tradeDefinition.MaxTradesPerStrategy = Convert.ToInt32(tbMaxOrders.Text); }
            catch { }
            ...
            try { tradeDefinition.LimitOffset = Convert.ToDouble(tbOffset.Text); }
            catch { }
            try { tradeDefinition.GoodForSecs = Convert.ToInt32(tbGoodForSecs.Text); }
            catch { }
